...
public interface DaysMealsRepository extends Neo4jRepository<MealPlanModel, String> {
    @Query("MATCH (d:DaysMeals) WHERE d.mealDate >= $startDate AND d.mealDate <= datetime($startDate) + duration('P4D') RETURN d")
    List<MealPlanModel> findMealsForNextWeek(DayOfWeek startDate);

    @Query("MATCH (d:DaysMeals {mealDate: $mealDate}) RETURN d LIMIT 1")
    Optional<MealPlanModel> findByMealDate(DayOfWeek mealDate);

    @Query("MATCH (d:DaysMeals {mealDate: $mealDate}) RETURN d")
    Optional<MealPlanModel> findMealsForDate(DayOfWeek mealDate);
