...
public interface MealRepository extends Neo4jRepository<MealModel, String> {
        ...
        @Query("MATCH (u:User {email: $email})-[r:HAS_MEAL {date: date($date), mealType: $type}]->(m:Meal {name: $oldMeal}) DELETE r WITH u MATCH (m2: Meal {name: $newMeal}) MERGE (u)-[r: HAS_MEAL {date: date($date), mealType: $type}]->(m2) RETURN m2.name AS name, m2.instructions AS instructions, m2.description AS description, "
                        +
                        "m2.image AS image, m2.ingredients AS ingredients, m2.cookingTime AS cookingTime, m2.type AS type")
        MealModel replaceMeal(LocalDate date, String oldMeal, String newMeal, String email, String type);
}
